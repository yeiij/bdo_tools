...
from tkinter import BOTTOM, RIGHT, X, Frame, Label, Menu, StringVar, TclError, Tk, PhotoImage, messagebox
...
from core.optimizer import clear_cache
...
class AppWindow:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            # src/gui -> src -> resources -> icon.png
            icon_path = current_dir.parent.parent / "resources" / "icon.png"
            if icon_path.exists():
                self.root.iconphoto(False, PhotoImage(file=str(icon_path)))
        except Exception:
            # Fallback or ignore if icon missing
            pass
        ...
        # New detailed network table
        network_table = NetworkTableSection(process_name)
    ...
    def _create_menu(self) -> None:
        ...
        tools_menu.add_command(label="Clear Game Cache", command=self._on_clear_cache)
        menubar.add_cascade(label="Tools", menu=tools_menu)

    def _on_clear_cache(self) -> None:
        if not messagebox.askyesno(
            "Clear Cache",
            "This will delete the 'UserCache' and 'Cache' folders from your Documents/Black Desert directory.\n\n"
            "You should close the game before proceeding.\n\n"
            "Continue?",
            parent=self.root
        ):
            return

        success, msg = clear_cache()
        if success:
